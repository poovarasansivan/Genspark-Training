...
namespace MigratedApi.Controllers
{
    ...
    public class ProductController : ControllerBase
    {
        ...
        [HttpGet("pagination")]
        [Authorize]
        public async Task<IActionResult> GetProductsByPagination([FromQuery] ProductPaginationDto productPaginationDto)
        {
            try
            {
                var products = await _productService.GetProductsByPagination(productPaginationDto);
                return Ok(SuccessResponseHandler.Success(products, "Products retrieved successfully."));
            }
            catch (ArgumentException ex)
            {
