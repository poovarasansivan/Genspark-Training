...
namespace MigratedApi.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        [HttpGet("paginated")]
        [Authorize]
        public async Task<IActionResult> GetPaginatedOrders([FromQuery] OrdersPaginationDto paginationDto)
        {
            try
            {
                var paginatedOrders = await _orderService.GetPaginatedOrdersAsync(paginationDto);
                return Ok(SuccessResponseHandler.Success(paginatedOrders, "Paginated orders retrieved successfully."));
            }
            catch (ArgumentException ex)
            {
                return BadRequest(ex.Message);
